...
namespace EdFi.OdsApi.Api.Resources 
{
    public class SectionsApi 
    {
        ...
        /// <returns>A RestSharp <see cref="IRestResponse"/> instance containing the API response details.</returns>
        public IRestResponse<Section> GetSectionByKey(string classPeriodName, string classroomIdentificationCode, string localCourseCode, int? schoolId, int? schoolYear, int? sequenceOfCourse, string termDescriptor, string uniqueSectionCode, string IfNoneMatch= null) 
        {
            var request = new RestRequest("/sections", Method.GET);
            request.RequestFormat = DataFormat.Json;

            // verify required params are set
            if (classPeriodName == null || classroomIdentificationCode == null || localCourseCode == null || schoolId == null || schoolYear == null || sequenceOfCourse == null || termDescriptor == null || uniqueSectionCode == null ) 
